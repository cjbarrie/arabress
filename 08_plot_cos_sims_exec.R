...
all_data <- bind_rows(lapply(versions, function(version) {
  bind_rows(lapply(countries, function(country) {
    cos_simsdf <-
      readRDS(paste0("data/output/cos_sims/", country, "/", "cos_simsdf_all_exec", version, ".rds"))
    cos_simsdf %>%
...
ggsave("plots/combinedplot_all_exec.png", units = "in", 
...
ggsave(
  "plots/combinedplot_top_exec.png",
  units = "in",
  width = 20,
  height = 5,
  dpi = 300
