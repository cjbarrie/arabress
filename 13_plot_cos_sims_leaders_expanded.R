...
all_data <- bind_rows(lapply(versions, function(version) {
  bind_rows(lapply(countries, function(country) {
    cos_simsdf <-
      readRDS(paste0("data/output/cos_sims_expanded/", country, "/", "cos_simsdf_all", version, ".rds"))
    cos_simsdf %>%
...
ggsave("plots/combinedplot_all_expanded.png", units = "in",
...
ggsave(
  "plots/combinedplot_top_expanded.png",
  units = "in",
  width = 20,
  height = 5,
  dpi = 300
